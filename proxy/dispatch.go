...
func (session *SessionConnection) processHttpEvent(ev *event.HTTPRequestEvent) error {
	...
	manager := registedRemoteConnManager[C4_NAME]

	var err error
	if session.RemoteConn != nil {
		err, _ = session.RemoteConn.Request(session, ev)
	} else {
		session.RemoteConn, err = manager.GetRemoteConnection(ev, attrs)
		if err != nil {
			log.Printf("Session[%d][WARN][%s]Failed to request proxy event for reason:%v", session.SessionID, manager.GetName(), err)
		} else {
			err, _ = session.RemoteConn.Request(session, ev)
		}
	}
