...
func (r *rangeBody) WriteHttpBody(body io.Reader) int {
	...
	var n int
	tmpbuf, ok := body.(*BufferCloseWrapper)
	if ok {
		n = tmpbuf.Buf.Len()
		r.c <- tmpbuf.Buf
	} else {
...
func (r *rangeFetchTask) processRequest(req *http.Request) error {
	...
	if len(rangeHeader) > 0 {
		...
		r.contentBegin, r.contentEnd = ParseRangeHeaderValue(rangeHeader)
...
func (r *rangeFetchTask) processResponse(res *http.Response) error {
	...
	case STATE_WAIT_HEAD_RES:
		...
		if len(contentRangeHeader) > 0 {
			_, _, length := ParseContentRangeHeaderValue(contentRangeHeader)
		...
		if nil != resbody && r.res.StatusCode < 300 {
			...
			tmpbuf, ok := resbody.(*BufferCloseWrapper)
			if ok {
				n = tmpbuf.Buf.Len()
				rb.buf = tmpbuf.Buf
			} else {
	...
	case STATE_WAIT_RANGE_GET_RES:
		...
		start, _, _ := ParseContentRangeHeaderValue(contentRange)
